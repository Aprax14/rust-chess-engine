...
impl BBPosition {
    ...
    pub fn piece_at(&self, square: Bitboard) -> Option<Piece> {
    ...
    pub fn available_moves(&self, piece: Piece, piece_position: Bitboard) -> Bitboard {
        let occupied = self.occupied_cells();
        let our_squares = self.occupied_by(piece.color);
        let enemies = self.occupied_by(piece.color.other());
        match (piece.kind, piece.color) {
    ...
    pub fn square_is_defended_by(&self, square: &Bitboard, color: Color) -> bool {
        self.defended_squares(color) & *square != Bitboard::new(0)
    }
            ...
            MoveKind::Standard { from, to } => {
                ...
                *piece_bitboard = *piece_bitboard & !from;
                ...
                if let Some(oc) = self.piece_at(to) {
                    resulting_bitboards.get_mut(oc).bits &= !to.bits;
                }
